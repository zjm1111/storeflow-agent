...
try:
    ...
    def run_task(self, task_id: str, checkpoint_version: int | None = None, workspace_id: str = "demo") -> None:
        from app.services.research import ResearchService
        task = ResearchService().get(task_id, workspace_id)
        if task is None or (checkpoint_version is not None and task.get("checkpoint", {}).get("version", 0) != checkpoint_version):
            return
